...
for row=1:512
  ...
  %level5 divide lower left-hand subband into 4 subbands creating 19 subbands
  X_new=analysis_filter_rows(X_new,32,32,1,1,h1,g1);
  X_new=analysis_filter_columns(X_new,16,32,1,1,h1,g1);
  X_new=analysis_filter_columns(X_new,16,32,16,1,h1,g1);
  
  X_new_1=zeroed_16(X_new,1);
  X_new_3=zeroed_16(X_new,3);
  X_new_6=zeroed_16(X_new,6);
  X_new_1=quantization_16band(X_new_1);
  X_new_3=quantization_16band(X_new_3);
  X_new_6=quantization_16band(X_new_6);
  X_new_1=zeroed_16(X_new_1,1);
  X_new_3=zeroed_16(X_new_3,3);
  X_new_6=zeroed_16(X_new_6,6);
  
  
  %%%%synthesis
  % synthesis level5
  Y_rec=synthesis_filter_columns(X_new_1,rows/32,columns/32,1,1,h2,g2);
  Y_rec=synthesis_filter_columns(Y_rec,rows/32,columns/32,rows/32,1,h2,g2);
  Y_rec=synthesis_filter_rows(Y_rec,rows/32,columns/16,1,1,h2,g2);
  
  % synthesis level4 
  Y_rec=synthesis_filter_columns(Y_rec,32,32,1,1,h2,g2);
...
%level2 upper left
 ...
 %level1
  ...
  psnr1=calc_psnr(Y_rec,X);% calc_psnr funtion has been defined with the basic SNR formula and returns the PSNR.
  ...
  subplot(2,4,2);
  imshow(Y_rec,[]);
  title('Reconstructed Image1(Highest freq-Zero)')
  
  %%%%synthesis
  % synthesis level5
  Y_rec=synthesis_filter_columns(X_new_3,rows/32,columns/32,1,1,h2,g2);
  Y_rec=synthesis_filter_columns(Y_rec,rows/32,columns/32,rows/32,1,h2,g2);
  Y_rec=synthesis_filter_rows(Y_rec,rows/32,columns/16,1,1,h2,g2);
...
%level2 upper left
 ...
 %level1
  ...
  %%%%synthesis
  % synthesis level5
  Y_rec=synthesis_filter_columns(X_new_6,rows/32,columns/32,1,1,h2,g2);
  Y_rec=synthesis_filter_columns(Y_rec,rows/32,columns/32,rows/32,1,h2,g2);
  Y_rec=synthesis_filter_rows(Y_rec,rows/32,columns/16,1,1,h2,g2);
...
%level2 upper left
 ...
 %level1
  ...
  psnr6=calc_psnr(Y_rec,X);
  subplot(2,4,4);
  imshow(Y_rec,[]);
  title('Reconstructed Image1(Six-highest freq-Zero)')
  
  %   Magnitude spectrum
X_fft=fft_basic(fft_basic(X).').';
X_fft_1=fft_basic(fft_basic(X_new_1).').';
X_fft_3=fft_basic(fft_basic(X_new_3).').';
X_fft_6=fft_basic(fft_basic(X_new_6).').';
subplot(2,4,5)
imshow(log(fftshift(abs(X_fft))),[])
title('2D-DFT Magnitude Spectrum of Original Lina Image')
subplot(2,4,6)
imshow(log(fftshift(abs(X_fft_1))),[])
...
subplot(2,4,8)
imshow(log(fftshift(abs(X_fft_6))),[])
title('2D-DFT Magnitude Spectrum of Reconstructed image3')

function X_new= zeroed_16(X_new, num)
if num==1
    for i=257:512
        for j=257:512
            X_new(i,j)=0;
...
function quant_16 = quantization_16band(ac)
...
for i=1:512
    for j=1:512
        ...
        quant_16(i,j)=((temp_ac*step_ac+minimum_ac)+(temp_ac*step_ac+minimum_ac))/2;
    end
end
end
